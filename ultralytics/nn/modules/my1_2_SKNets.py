...
class SKAttention(nn.Module):
    ...
    def __init__(self, channel=512,kernels=[1,3,5,7],reduction=8,group=1,L=32): # 4个卷积核
        super().__init__()
        ...
        for k in kernels:
            self.convs.append(
                nn.Sequential(OrderedDict([
                    ('conv',nn.Conv2d(channel,channel,kernel_size=k,padding=k//2,groups=group)), # padding = k // 2 保证: in_c = out_c
                    ('bn',nn.BatchNorm2d(channel)), # 批量归一化
                    ('relu',nn.ReLU())
                ]))
            )
        # 将全局向量降维
        self.fc=nn.Linear(channel,self.d) # 全局池化
...
###使用 torch.stack()函数和sum()函数进行升维度和降维度的操作
    ...
    def forward(self, x):
        ...
        #conv_outs的长度等于卷积核的长度
        feats=torch.stack(conv_outs,0) # 将K个尺度的输出在第0个维度上拼接: (K,B,C,H,W)

        # Fuse: 首先将多尺度的信息进行相加,sum()默认在第一个维度进行求和 : sum()直接降维
